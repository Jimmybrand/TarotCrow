"""Domain view models for tarot cards.

These classes are a direct move of the `Card` / `PipCard` / `TrumpCard`
hierarchy previously located in `src/tarot/core.py`. They exist purely as
a domain-layer view and intentionally do not alter the canonical
`TarotCard` model in `src/tarot/canon/models.py`.
"""
